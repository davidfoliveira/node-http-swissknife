...
exports.url = function(opts) {
    ...
    else if ( url.match(/^(GET|POST) +([^ ]+) +HTTP\//i) ) {
    ...
    return _forLimit(opts.number,opts.concurrents,
        function(x,next){
            ...
            return httpc.request(_url,{headers: opts.header},function(err,dataSize,res){
                var
        ...
        function(err,res){
            totalTimeSpent = (new Date().getTime() - start) / 1000;
...
exports.file = function(opts) {
    ...
    requests = new RequestStream(opts.concurrents, opts);
    ...
    rl.on('line', function(line){
        ...
        else if ( line.match(/^(GET|POST) +([^ ]+) +HTTP\//i) ) {
