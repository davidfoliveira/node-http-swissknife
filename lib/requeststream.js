...
function RequestStream(concurrents,opts) {
...
RequestStream.prototype._performRequest = function(url,opts) {
    ...
    self.currentReqs++;
    self.done++;
    console.log("Sending req: ", url.path);
    return httpc.request(url,{headers: self.opts.header},function(err,dataSize,res){
        var
...
RequestStream.prototype._nextRequest = function(opts) {
    ...
    if ( self.queue.length == 0 ) {
        ...
        if ( self.currentReqs == 0 ) {
            self.totalTimeSpent = (new Date().getTime() - self.started) / 1000;
            self.emit('complete');
        }
        return;
    }
